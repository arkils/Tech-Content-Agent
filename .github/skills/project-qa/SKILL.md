...
Set `ENABLED_PUBLISHERS=blog,linkedin` and create the `/tech-news-agent/linkedin` parameter in AWS SSM Parameter Store (SecureString) with `{"access_token": "...", "author_urn": "urn:li:person:..."}`. See [Deployment Q&A](./references/deployment-qa.md).
...
**Never in code.** All credentials are stored in AWS SSM Parameter Store as SecureString parameters. Parameter paths are constants in `AgentConfig` (e.g. `AgentConfig.LINKEDIN_PARAM_PATH`). See [Deployment Q&A](./references/deployment-qa.md).
...
AgentCore (agent runtime), EventBridge (scheduling), Bedrock (LLM), DynamoDB (dedup state), SSM Parameter Store (credentials), CloudWatch (logs/metrics), IAM (least-privilege roles). See [Architecture Q&A](./references/architecture-qa.md).
